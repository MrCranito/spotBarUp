...
class UserServiceImp: UserService {
    ...
    func getUser(params: User, completion: @escaping (User?, Error?) -> ()) {
        print("ok")
        Alamofire.request("http://127.0.0.1:6003/auth/getUser", method: .get, parameters: ["email": params.email ?? "email"], encoding: JSONEncoding.default, headers: nil).responseString{ (data) in
            NSLog("\(String(describing: data.result.value))")
    ...
    func checkSchema(params: checkSchema, completion: @escaping (checkSchema?, Error?) -> ()) {
        print("ok here too")
        Alamofire.request("http://127.0.0.1:6003/auth/testConnect", method: .post, parameters: ["country": params.country ?? "country", "state": params.state ?? "state"], encoding: JSONEncoding.default, headers: nil).responseString{ (data) in
            NSLog("\(String(describing: data.result.value))")
